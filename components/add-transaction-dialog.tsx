      ...
      reader.onload = async (event) => {
        ...
        if (isImage) {
          ...
          try {
            ...
            // 2. Отправляем текст и картинку на сервер
            // Сервер сначала попробует распарсить текст от Tesseract.
            // Если не получится - отправит картинку в Mistral (если есть ключ).
            const response = await fetch('/api/analyze-image', {
              method: 'POST',
              headers: { 'Content-Type': 'application/json' },
              body: JSON.stringify({
                text: ocrText,
                image: fileUrl, // Отправляем картинку для Mistral fallback
              }),
            });
